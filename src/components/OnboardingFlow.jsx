...
export default function OnboardingFlow({ onComplete }) {
    ...
    const handleFinalSubmit = async () => {
        ...
        try {
          ...
          // 1) Create auth user (or login if email already exists)
          try {
            uid = await registerUser(formData.email, formData.password, {
              firstName: formData.firstName,
              lastName: formData.lastName,
              LMP: formData.lmpDate,
            });
          } catch (err) {
            if (err.code === "auth/email-already-in-use") {
              uid = await loginUser(formData.email, formData.password);
            } else {
              throw err;
            }
          }
      
          // 2) Save main profile doc (optional, since registerUser already writes ProfileUsers/{uid})
          ...
          // 3) Subcollections
          await setPreviousPregnancies(uid, {
            isFirstTime: formData.isFirstPregnancy,
            previousPregnancies: formData.isFirstPregnancy ? 0 : formData.pregnancyCount,
          });
      
          // IMPORTANT: your firebase.js expects `conditions` (lowercase), not `Conditions`
          const finalConditions = formData.hasConditions
            ? [
                ...formData.conditions.filter((c) => c !== "Other"),
                ...(formData.conditions.includes("Other") && formData.otherCondition
                  ? [formData.otherCondition]
                  : []),
              ]
            : [];
      
          await setPreExistingConditions(uid, {
            hasConditions: formData.hasConditions,
            conditions: finalConditions,
          });
      
          // IMPORTANT: your firebase.js computes hasVillage from village, so you only need village
          await setSupportSystem(uid, {
            village: formData.supportSystem,
          });
      
          message.success("Profile Created Successfully!");
          onComplete({ ...formData, uid });
        } catch (err) {
          console.error(err);
          message.error(err?.message || "Failed to create profile. Please try again.");
        } finally {
          setIsSaving(false);
        }
      };
